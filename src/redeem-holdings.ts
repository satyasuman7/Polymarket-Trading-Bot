...
function appendPnlLogLine(line: string): void {
    ...
    try {
        ...
        console.log(`[WARNING] Failed to append pnl.log: ${e instanceof Error ? e.message : String(e)}`);
    }
}
...
function ensurePnlLogExists(): void {
    ...
    try {
        ...
        console.log(`[WARNING] Failed to ensure pnl.log exists: ${e instanceof Error ? e.message : String(e)}`);
    }
}
...
function loadCopytradeStateFile(): CopytradeStateFile {
    ...
    try {
        ...
        console.log(`[WARNING] Failed to read copytrade-state.json for pnl: ${e instanceof Error ? e.message : String(e)}`);
        return {};
    }
}
...
function pruneCopytradeStateKeepNewest(keep: number): void {
    ...
    try {
        ...
        console.log(`[WARNING] Failed to read copytrade-state.json for pruning: ${e instanceof Error ? e.message : String(e)}`);
        return;
    }
    ...
    try {
        ...
        console.log(`[INFO] Pruned copytrade-state.json removed=${removed} kept=${Object.keys(pruned).length}`);
    } catch (e) {
        console.log(`[WARNING] Failed to write pruned copytrade-state.json: ${e instanceof Error ? e.message : String(e)}`);
    }
}
...
async function main() {
        ...
        try {
            ...
            if (!dryRun) {
                ...
                try {
                    ...
                    console.log(`[WARNING] Failed to get wallet address for balance logging: ${e instanceof Error ? e.message : String(e)}`);
                }
                            ...
                            if (walletAddress) {
                                try {
                                    balanceAfterRedeem = await getUsdcBalance(walletAddress);
                                     ...
                                     console.log(`[WARNING] Failed to get balance after redeem for conditionId=${r.conditionId}: ${e instanceof Error ? e.message : String(e)}`);
                                }
                            }
                            await recordPnlForRedeemedCondition(r.conditionId, balanceAfterRedeem);
                            wroteAny = true;
                        } catch (e) {
                            console.log(`[WARNING] Failed to record pnl for conditionId=${r.conditionId}: ${e instanceof Error ? e.message : String(e)}`);
                        }
                    }
                }
            ...
            for (const r of result.results) {
                if (r.isResolved && !r.redeemed && shouldDropHoldingsForError(r.error)) {
                    console.log(`[WARNING] Dropping holdings for conditionId=${r.conditionId} (no winning tokens to redeem)`);
                    try {
                        clearMarketHoldings(r.conditionId);
                    } catch (e) {
                        console.log(`[ERROR] Failed to clear holdings for ${r.conditionId}`, e);
                    }
                }
            }
            ...
            console.log(`[ERROR] redeem-holdings run failed`, e);
        } finally {
            running = false;
        }
    };
    ...
    console.log(`[INFO] redeem-holdings loop enabled intervalMs=${intervalMs} initialDelayMs=${initialDelayMs} align=${align} keepState=${keepState} (first run in ${Math.ceil(waitMs / 1000)}s) dryRun=${dryRun} maxRetries=${maxRetries}`);
    // await sleep(waitMs);
    await runOnce();
...
main().catch((e) => {
    console.log(`[ERROR] Fatal error in redeem-holdings`, e);
    process.exit(1);
});
