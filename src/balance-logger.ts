...
function ensureBalanceLogExists(): void {
    ...
    if (!fs.existsSync(logPath)) {
        ...
        console.log(`[INFO] Created balance log file: ${BALANCE_LOG_FILE}`);
    }
}
...
async function logBalanceForMarket(): Promise<void> {
    try {
        ...
        console.log(`[SUCCESS] 💰 Balance logged: ${balance.toFixed(6)} USDC (${marketSlug})`);
    } catch (error) {
        console.log(`[ERROR] Failed to log balance: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
async function main(): Promise<void> {
    console.log(`[INFO] ═══════════════════════════════════════════════════`);
    console.log(`[INFO] 💰 Balance Logger Started`);
    console.log(`[INFO] ═══════════════════════════════════════════════════`);
    console.log(`[INFO] Log file: ${BALANCE_LOG_FILE}`);
    console.log(`[INFO] Wallet: ${new Wallet(config.requirePrivateKey()).address}`);
    console.log(`[INFO] ═══════════════════════════════════════════════════`);
    ...
    if (isOnce) {
        // Run once and exit
        console.log(`[INFO] Running once (--once flag)`);
        await logBalanceForMarket();
        console.log(`[INFO] Balance logged successfully. Exiting.`);
        return;
    }
    ...
    console.log(`[INFO] ⏰ Waiting for next 15-minute boundary...`);
    console.log(`[INFO]    Current time: ${new Date().toISOString()}`);
    console.log(`[INFO]    Next boundary: ${nextBoundary.toISOString()}`);
    console.log(`[INFO]    Wait time: ${Math.ceil(msUntilNext / 1000)}s`);
    ...
    console.log(`[INFO] ✅ Balance logger running (every 15 minutes)`);
    console.log(`[INFO]    Next log at: ${new Date(Date.now() + intervalMs).toISOString()}`);
}
...
// Run main function
main().catch((error) => {
    console.log(`[ERROR] Fatal error in balance logger`, error);
    process.exit(1);
});
