...
function loadState(): CopytradeStateFile {
    ...
    try {
        ...
        console.log(`[WARNING] Failed to read legacy state: ${e instanceof Error ? e.message : String(e)}`);
        return {};
    }
}
...
export class CopytradeArbBot {
    ...
    private async tickMarket(market: string): Promise<void> {
        ...
        if (maxAttemptsReached) {
            ...
            if (!this.hedgedLoggedSlugs.has(slug)) {
                ...
                console.log(`[INFO] ✅ Hedge complete: market=${market} slug=${slug} attempts=${row.attemptCountYES}Y/${row.attemptCountNO}N successful=${row.buyCountYES}Y/${row.buyCountNO}N avgYES=${avgYes.toFixed(4)} avgNO=${avgNo.toFixed(4)} sumAvg=${(avgYes + avgNo).toFixed(4)}`);
            }
        ...
        if (tracking.isNewHedge && tracking.firstBuyOfHedge) {
            ...
            if (!yesBelow && !noBelow) {
                ...
                if (config.debug) {
                    console.log(`[DEBUG] ⏳ Waiting for entry (new hedge): market=${market} YES=${upMid.toFixed(4)} NO=${downMid.toFixed(4)} threshold=${this.cfg.threshold}`);
                }
                return;
            }
            ...
            if (row.lastBuySide === "YES" && yesBelow) {
                selectedToken = "YES";
                console.log(`[INFO] 🎯 New hedge entry (flexible): Last buy was YES, YES below threshold (${upMid.toFixed(4)} ≤ ${this.cfg.threshold}), starting with YES`);
            } else if (row.lastBuySide === "NO" && noBelow) {
                selectedToken = "NO";
                console.log(`[INFO] 🎯 New hedge entry (flexible): Last buy was NO, NO below threshold (${downMid.toFixed(4)} ≤ ${this.cfg.threshold}), starting with NO`);
            } else if (yesBelow && noBelow) {
                selectedToken = "YES"; // Default priority if both below
                console.log(`[INFO] 🎯 New hedge entry (flexible): Both below threshold, starting with YES (priority) price=${upMid.toFixed(4)}`);
            } else if (yesBelow) {
                selectedToken = "YES";
                console.log(`[INFO] 🎯 New hedge entry (flexible): YES below threshold, starting with YES price=${upMid.toFixed(4)}`);
            } else {
                selectedToken = "NO";
                console.log(`[INFO] 🎯 New hedge entry (flexible): NO below threshold, starting with NO price=${downMid.toFixed(4)}`);
            }
            ...
            if (!yesBelow && !noBelow) {
                // PERFORMANCE OPTIMIZATION: Only log when DEBUG enabled
                if (config.debug) {
                    console.log(`[DEBUG] ⏳ Waiting for entry: market=${market} YES=${upMid.toFixed(4)} NO=${downMid.toFixed(4)} threshold=${this.cfg.threshold}`);
                }
                return;
            }
            ...
            if (yesBelow && noBelow) {
                ...
                console.log(`[INFO] 🎯 Entry signal: Both below threshold, tracking YES first (priority) price=${upMid.toFixed(4)}`);
            } else if (yesBelow) {
                tracking.trackingToken = "YES";
                tracking.tempPrice = upMid;
                console.log(`[INFO] 🎯 Entry signal: YES below threshold, tracking YES price=${upMid.toFixed(4)}`);
            } else {
                tracking.trackingToken = "NO";
                tracking.tempPrice = downMid;
                console.log(`[INFO] 🎯 Entry signal: NO below threshold, tracking NO price=${downMid.toFixed(4)}`);
            }
            tracking.initialized = true;
            tracking.firstBuyOfHedge = true;
        }
        ...
        if (attemptCount >= this.cfg.maxBuysPerSide) {
            ...
            // Switch to opposite token to continue building hedge
            console.log(`[INFO] 🔄 ${currentToken} side reached max attempts (${attemptCount}/${this.cfg.maxBuysPerSide}), switching to ${oppositeToken} @ ${oppositePrice.toFixed(4)}`);
            tracking.trackingToken = oppositeToken;
            tracking.tempPrice = oppositePrice;
            tracking.secondSideTimerSessionStart = null;
            tracking.secondSideTimerAccumulated = 0;
        ...
        if (currentPrice < tracking.tempPrice) {
            ...
            if (config.debug) {
                if (isAcceptable) {
                    console.log(`[INFO] 📉 Price DROP: ${currentToken} ${tracking.tempPrice.toFixed(4)} → ${currentPrice.toFixed(4)} (temp updated ✅, max acceptable: ${maxAcceptablePrice.toFixed(4)}, current sumAvg: ${(currentAvgYES + currentAvgNO).toFixed(4)})`);
                } else {
                    console.log(`[INFO] 📉 Price DROP but still too high: ${currentToken} ${tracking.tempPrice.toFixed(4)} → ${currentPrice.toFixed(4)} (temp updated, max acceptable: ${maxAcceptablePrice.toFixed(4)}, waiting for price ≤ ${maxAcceptablePrice.toFixed(4)})`);
                }
            }
            return;
        }
        ...
        if (isSecondSide) {
            ...
            if (currentPrice > dynamicThreshold) {
                ...
                if (tracking.secondSideTimerSessionStart !== null) {
                    tracking.secondSideTimerSessionStart = null;
                    if (config.debug) {
                        console.log(`[DEBUG] 📊 Second side: Price ${currentPrice.toFixed(4)} > threshold ${dynamicThreshold.toFixed(4)}, resetting timer`);
                    }
                }
                ...
                if (tracking.secondSideTimerSessionStart === null) {
                    ...
                    if (config.debug) {
                        console.log(`[DEBUG] 📊 Second side: Price ${currentPrice.toFixed(4)} <= threshold ${dynamicThreshold.toFixed(4)}, starting continuous timer`);
                    }
                }
        ...
        if (isTimeBasedBuy && !recentlyFailed) {
            ...
            console.log(`[SUCCESS] 🎯 SECOND BUY (Time-based): ${currentToken} @ ${currentPrice.toFixed(4)} | Price <= threshold ${tracking.tempPrice.toFixed(4)} for ${timeBelowThreshold}ms (>= ${this.cfg.secondSideTimeThresholdMs}ms) attempt_count=${attemptCount + 1}/${this.cfg.maxBuysPerSide}`);
            ...
            if (buyPrice !== null) {
                ...
                if (tracking.firstBuyOfHedge) {
                    ...
                    console.log(`[SUCCESS] 🎯 FIRST BUY: ${currentToken} @ ${actualBuyPrice.toFixed(4)} | Hedge started`);
                }
                ...
                if (actualBuyPrice !== buyPrice && config.debug) {
                    console.log(`[DEBUG] 📊 Using actual fill price ${actualBuyPrice.toFixed(4)} (vs limit ${buyPrice.toFixed(4)}) for dynamic threshold`);
                }
                ...
                if (config.debug) {
                    console.log(`[INFO] 📊 Dynamic threshold (next side): 1 - ${buyPrice.toFixed(4)} = ${dynamicThreshold.toFixed(4)} (will buy ${oppositeToken} immediately when price <= ${(dynamicThreshold - this.cfg.secondSideBuffer).toFixed(4)} = ${dynamicThreshold.toFixed(4)} - ${this.cfg.secondSideBuffer.toFixed(4)} buffer)`);
                    if (wasClampedToMin) {
                        const minOrderValue = (calculatedTempPrice + estimatedPriceBuffer) * this.cfg.sharesPerSide;
                        console.log(`[INFO] ⚠️ Threshold adjusted for $1 minimum order: ${dynamicThreshold.toFixed(4)} → ${calculatedTempPrice.toFixed(4)} (min threshold: ${minAcceptableThreshold.toFixed(4)} to ensure order value ≥ $1, estimated order value: $${minOrderValue.toFixed(2)} = (${calculatedTempPrice.toFixed(4)} + ${estimatedPriceBuffer.toFixed(4)}) × ${this.cfg.sharesPerSide})`);
                    } else if (wasClampedToMax) {
                        console.log(`[INFO] ⚠️ Threshold clamped to max: ${dynamicThreshold.toFixed(4)} → ${calculatedTempPrice.toFixed(4)}`);
                    }
                    console.log(`[INFO] 🎯 New tracking price for ${oppositeToken}: ${calculatedTempPrice.toFixed(4)} (immediate buy trigger: <= ${(calculatedTempPrice - this.cfg.secondSideBuffer).toFixed(4)})`);
                } else {
                    console.log(`[SUCCESS] 🎯 DYNAMIC THRESHOLD: ${dynamicThreshold.toFixed(4)} | Will buy ${oppositeToken} when <= ${(dynamicThreshold - this.cfg.secondSideBuffer).toFixed(4)}`);
                }
            }
            if (!buySucceeded) {
        ...
        if (isDeepDiscount && !recentlyFailed) {
            ...
            console.log(`[SUCCESS] 🎯 SECOND BUY (Depth-based): ${currentToken} @ ${currentPrice.toFixed(4)} | Dropped ${discountPercent}% below tempPrice (${tracking.tempPrice.toFixed(4)} → ${currentPrice.toFixed(4)}, threshold: ${(this.cfg.depthBuyDiscountPercent * 100).toFixed(1)}%) attempt_count=${attemptCount + 1}/${this.cfg.maxBuysPerSide}`);
            ...
            if (buyPrice !== null) {
                ...
                // PERFORMANCE OPTIMIZATION: Only log detailed threshold calculation when DEBUG enabled
                if (config.debug) {
                    console.log(`[INFO] 📊 Dynamic threshold (second side): 1 - ${buyPrice.toFixed(4)} = ${dynamicThreshold.toFixed(4)} (will buy ${oppositeToken} immediately when price <= ${(dynamicThreshold - this.cfg.secondSideBuffer).toFixed(4)} = ${dynamicThreshold.toFixed(4)} - ${this.cfg.secondSideBuffer.toFixed(4)} buffer)`);
                    if (wasClampedToMin) {
                        const minOrderValue = (calculatedTempPrice + estimatedPriceBuffer) * this.cfg.sharesPerSide;
                        console.log(`[INFO] ⚠️ Threshold adjusted for $1 minimum order: ${dynamicThreshold.toFixed(4)} → ${calculatedTempPrice.toFixed(4)} (min threshold: ${minAcceptableThreshold.toFixed(4)} to ensure order value ≥ $1, estimated order value: $${minOrderValue.toFixed(2)} = (${calculatedTempPrice.toFixed(4)} + ${estimatedPriceBuffer.toFixed(4)}) × ${this.cfg.sharesPerSide})`);
                    } else if (wasClampedToMax) {
            ...
            console.log(`[SUCCESS] ${buySideLabel}: ${currentToken} @ ${currentPrice.toFixed(4)} (reversed from ${tracking.tempPrice.toFixed(4)}, movement=+${actualPriceMovement.toFixed(4)}, threshold=${reversalThreshold.toFixed(4)}) attempt_count=${attemptCount + 1}/${this.cfg.maxBuysPerSide}`);
            ...
            if (buyPrice !== null) {
                ...
                if (config.debug) {
                    ...
                    console.log(`[INFO] 🎯 New tracking price for ${oppositeToken}: ${calculatedTempPrice.toFixed(4)}`);
                } else {
                    console.log(`[SUCCESS] 🎯 DYNAMIC THRESHOLD: ${dynamicThreshold.toFixed(4)} | Will buy ${oppositeToken} when <= ${(dynamicThreshold - this.cfg.secondSideBuffer).toFixed(4)}`);
                }
            } 
    ...
    private async buySharesLimit(
        ...
        if (orderValue < 1.0) {
            ...
            console.log(`[INFO] 💰 Adjusted shares to meet $1 minimum: ${size} → ${adjustedSize} shares (order value: $${orderValue.toFixed(2)} → $${adjustedOrderValue.toFixed(2)} at price=${limitPrice.toFixed(4)})`);
        }
        ...
        if (projectedSumAvg > this.cfg.maxSumAvg) {
            console.log(`[WARNING] Copytrade: market=${market} slug=${slug} skipping ${leg} buy at ${limitPrice.toFixed(4)} - would make sumAvg ${projectedSumAvg.toFixed(4)} > ${this.cfg.maxSumAvg} (unprofitable). Current: avgYES=${avg(row.costYES, row.qtyYES).toFixed(4)} avgNO=${avg(row.costNO, row.qtyNO).toFixed(4)}`);
        ...
        try {
            ...
            console.log(`[INFO] ⚡ Order placed in ${orderPlaceTime}ms`);
        ...
        if (this.cfg.fireAndForget) {
            ...
            console.log(`[SUCCESS] ⚡ Order ${orderID.substring(0, 20)}... placed and tracking async (total: ${totalTime}ms) - continuing immediately`);
